...
trait WmsTrait {
    ...
    public function subQueryWmsTransfer($charTable,$store_id,$type=''){
      $querySub = DB::table('wms_'.$charTable.'_details')
          ->selectRaw('sum(IFNULL(product_quantity,0)) as '.$type.'_quantity,product_id as '.$type.'_product_id')
          ->groupBy('product_id')
          ->join('wms_'.$charTable.'s','wms_'.$charTable.'s.id','=','wms_'.$charTable.'_details.'.$charTable.'_id');
      $querySub = $querySub->where('wms_'.$charTable.'s.'.$type.'_id','=',$store_id);
      return $querySub;
    }
    ...
    public function fullQueryWareHouse($request){
      $value = $this->queryProductNoParent()
          ...
          ->leftjoinSub($this->subQueryWmsTransfer('transfer',$request->store,'store_to'),'store_to_product_id',function($query){
            $query->on('products.id','=','store_to_product_id');
          })
          ->leftjoinSub($this->subQueryWmsTransfer('transfer',$request->store,'store_from'),'store_from_product_id',function($query){
